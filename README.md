...
### 1. Ⓨ Nano Banana 2 (生图/编辑)
*   **双模式智能切换**：
    *   **生图模式**：未连接参考图时，执行文生图 (Text-to-Image)。
    *   **编辑模式**：连接参考图 (image_1 ~ image_15) 时，自动切换为图片编辑/风格迁移模式。
*   **双模型支持**：内置 **Gemini 3 Pro** (画质顶尖) 和 **Gemini 3.1 Flash** (速度极快) 两种模型供选择。
*   **尺寸自动修正**：支持 1K/2K/4K 分辨率及多种宽高比。若模型返回尺寸不符，节点会自动进行**无损居中裁剪**和缩放，确保输出 100% 符合设定的像素尺寸。
*   **极速上传**：内置智能压缩算法，自动处理参考图大小，提升上传速度。
